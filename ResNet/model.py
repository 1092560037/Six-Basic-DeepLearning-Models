...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, **kwargs): #downsample对应虚线残差结构
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=out_channel, 
                               kernel_size=3, stride=stride, padding=1, bias=False) #使用BN时不需要设置偏置
        ...
        #stride=1时，对应实线结构
        #output = （input - 3 + 2*1）/1 + 1 = input，此时输出特征矩阵的高和宽与输入特征矩阵的高和宽相等
        #stride=2时，对应虚线结构
        # output = （input - 3 + 2*1）/2 + 1 = input/2 + 0.5 = input/2(向下取整)，此时输出特征矩阵的高和宽正好是输入特征矩阵的高和宽的一半向下取整
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = self.bn2(out) #此处没有激活函数，要加上捷径分支之后再使用激活函数
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None,
        ...
        # -----------------------------------------
        self.conv3 = nn.Conv2d(in_channels=width, out_channels=out_channel*self.expansion, #注意此处out_channel的变化
